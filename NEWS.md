...
* New `nv_scan()` runs a fixed-length loop that threads a carry through a
  step function while stacking each step's outputs, like JAX's `lax.scan`.
  Built on `nv_while()`; no new backend primitive.
