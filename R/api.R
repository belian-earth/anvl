...
nv_quantile <- function(x, probs, axis = NULL, interpolation = "linear", nan_rm = FALSE) {
  ...
  # values is at most ceil((n_valid - 1) * max(probs)) + 1, and n_valid <= n only
  # ever moves it down, so the ascending prefix of k_lo elements always holds
  # it; mirrored, the descending prefix of k_hi elements holds every position
  # from floor((n_valid - 1) * min(probs)) + 1 up. Either prefix comes from
  # top_k (of the negated values for the low end), which is cheaper than a sort
  # when the window is at most about half the axis. With nan_rm = FALSE, NaNs
  # rank to the front of the window instead of the back, but any slice
  ...
  # For float input, find NaN positions: nan_rm = TRUE sanitizes them to the
  # end the window does not read from (+Inf, or -Inf for the high window) so
  # the valid values keep their ranks; nan_rm = FALSE uses them post-hoc to
  # propagate.
  if (is_float) {
