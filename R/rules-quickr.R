...
quickr_stable_log1p_expr <- function(x_expr) {
  poly <- rlang::call2(
    "*",
    x_expr,
    rlang::call2(
  ...
  rlang::call2(
    ...
    rlang::call2("<", rlang::call2("abs", x_expr), 1e-5),
    poly,
    rlang::call2("log", rlang::call2("+", 1.0, x_expr))
  )
}
...
local({
  ...
  quickr_register_prim_lowerer(
    ...
    function(prim_name, inputs, params, out_syms, input_nodes, out_avals, ctx = NULL) {
      if (is.null(ctx)) {
        cli_abort("Internal error: missing quickr lowering context for primitive {.val scan}")
      }
      body_graph <- params$body_graph
      n <- as.integer(params$length)
      n_carry <- params$n_carry
      n_xs <- params$n_xs
      n_out <- length(out_syms) - n_carry
      carry_syms <- out_syms[seq_len(n_carry)]
      buf_syms <- out_syms[n_carry + seq_len(n_out)]
      buf_avals <- out_avals[n_carry + seq_len(n_out)]
      new_tmp_sym <- ctx$new_tmp_sym

      # The outputs double as the loop state: carry from the inputs, buffers
      # zeroed, then a for loop over the steps.
      stmts <- c(
        Map(function(lhs, rhs) rlang::call2("<-", lhs, rhs), carry_syms, inputs[seq_len(n_carry)]),
        Map(
          function(sym, aval) rlang::call2("<-", sym, quickr_alloc_zero(shape(aval), aval)),
          buf_syms,
          buf_avals
        )
      )
      ...
      i_sym <- new_tmp_sym()
      idx_sym <- new_tmp_sym()
      idx_expr <- if (isTRUE(params$reverse)) rlang::call2("-", n + 1L, i_sym) else i_sym
      loop <- list(rlang::call2("<-", idx_sym, idx_expr))

      x_syms <- vector("list", n_xs)
      for (k in seq_len(n_xs)) {
        node <- input_nodes[[n_carry + k]]
        shp <- as.integer(shape(node$aval))
        dt <- dtype(node$aval)
        slice_sym <- new_tmp_sym()
        x_sym <- new_tmp_sym()
        starts <- c(list(idx_sym), rep(list(1L), length(shp) - 1L))
        loop <- c(
          loop,
          quickr_emit_dynamic_slice(
            slice_sym,
            ...
            shp,
            c(1L, shp[-1L]),
            AbstractArray(dt, c(1L, shp[-1L]))
          ),
          quickr_emit_reshape(x_sym, slice_sym, c(1L, shp[-1L]), shp[-1L], AbstractArray(dt, shp[-1L]))
        )
        x_syms[[k]] <- x_sym
      }

      lowered <- quickr_lower_inline_graph(body_graph, c(carry_syms, x_syms), ctx)
      loop <- c(loop, lowered$stmts)
      carry_exprs <- lowered$out_exprs[seq_len(n_carry)]
      out_exprs <- lowered$out_exprs[n_carry + seq_len(n_out)]

      # Write this step's outputs before the carry moves on, since an output
      # may be the (old) carry itself.
      for (j in seq_len(n_out)) {
        aval <- body_graph$outputs[[n_carry + j]]$aval
        shp <- as.integer(shape(aval))
        upd_sym <- new_tmp_sym()
        loop <- c(
          loop,
          quickr_emit_reshape(upd_sym, out_exprs[[j]], shp, c(1L, shp), AbstractArray(dtype(aval), c(1L, shp))),
          quickr_emit_dyn_update_slice(
            buf_syms[[j]],
            buf_syms[[j]],
            upd_sym,
            c(list(idx_sym), rep(list(1L), length(shp))),
            shape(buf_avals[[j]]),
            c(1L, shp),
            buf_avals[[j]]
          )
      ...
      carry_tmp <- replicate(n_carry, new_tmp_sym(), simplify = FALSE)
      loop <- c(
        loop,
        Map(function(lhs, rhs) rlang::call2("<-", lhs, rhs), carry_tmp, carry_exprs),
        Map(function(lhs, rhs) rlang::call2("<-", lhs, rhs), carry_syms, carry_tmp)
      )

      loop_call <- as.call(list(
        as.name("for"),
        i_sym,
        rlang::call2("seq_len", n),
        as.call(c(list(as.name("{")), loop))
      ))
      c(stmts, list(loop_call))
    }
  )
